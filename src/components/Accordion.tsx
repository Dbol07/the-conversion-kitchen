...
export default function Accordion({ title, children, defaultOpen = false, icon = 'leaf' }: AccordionProps) {
  const [isOpen, setIsOpen] = useState(defaultOpen);

  const IconComponent = icon === 'teacup' ? TeacupIcon : icon === 'mushroom' ? MushroomIcon : LeafIcon;

  return (
    <div className="mb-4 parchment-card overflow-hidden animate-slide-up shadow-lg hover:shadow-xl transition-shadow">
      <button
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-6 py-4 flex items-center gap-3 text-left bg-gradient-to-r from-[#b8d3d5]/80 to-[#a77a72]/60 hover:from-[#b8d3d5] hover:to-[#a77a72]/80 transition-all"
      >
        <IconComponent />
        <h3 className="flex-1 text-lg font-semibold text-[#1b302c] font-serif">{title}</h3>
        <svg
          className={`w-5 h-5 text-[#5f3c43] transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
        >
